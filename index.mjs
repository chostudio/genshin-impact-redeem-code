...
try {
  ...
  // CHANGED: Point to the file in the current directory
  const rootPath = path.resolve(__dirname, './root.mjs')

  console.log('🚀 Running automation script from:', rootPath)

  const { default: runAutomation } = await import(rootPath)
  
  // Pass 'ai' AND 'secrets' to the automation
  await runAutomation({ 
    ai, 
